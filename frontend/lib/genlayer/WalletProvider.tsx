...
import { connectMetaMask, discoverMetaMask, formatAddress, GENLAYER_CHAIN_ID_HEX, getAccounts, getEthereumProvider } from "./client";
...
const DISCONNECT_KEY = "slashcourt.wallet.disconnected";
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    if (!provider || localStorage.getItem(DISCONNECT_KEY) === "true") {
      setConnecting(false);
      return;
    }
  ...
  return <WalletContext.Provider value={{ address, connected: Boolean(address), connecting, onGenLayer, connect, disconnect }}>{children}</WalletContext.Provider>;
}

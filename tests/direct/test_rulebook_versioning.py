...
def test_rulebook_versions_are_append_only_and_penalties_are_pinned(
    ...
    court.create_initial_rulebook(RULEBOOK_TEXT, "sha256:rulebook-v1")
    ...
    assert court.get_rulebook(1)["rulebook_hash"] == "sha256:rulebook-v1"
    assert court.get_rulebook(2)["rulebook_hash"] == "sha256:rulebook-v2"
    assert court.get_rulebook_versions(0, 10)["total"] == 2

    direct_vm.sender = direct_alice
    with direct_vm.expect_revert("governor authorization required"):
        court.publish_rulebook_version(RULEBOOK_TEXT, "sha256:attacker")
